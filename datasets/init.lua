...
-- it first creates a .t7 file in the generate folder(cache path) mentioned in opts
-- cachePath would have all the generated files after running -gen.lua file
   local cachePath = paths.concat(opt.gen, opt.dataset .. '.t7')
   if not paths.filep(cachePath) or not isvalid(opt, cachePath) then
      paths.mkdir('gen')
-- now it runs files like imagenet-gen.lua
